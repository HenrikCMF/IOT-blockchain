...
def on_message(client, userdata, msg):
    ...
    userid = payload[0]
    payload.pop(0)
    if str(msg.topic)=='New sensor':
        sensor_amount+=1
        New_sensor_topic='Name'
        message='a'+str(sensor_amount)+'/'
        client.publish(New_sensor_topic, message)
        client.subscribe(util.topic+message+"#")
        sensors.append(util.topic+message)
        return
    ...
    if str(msg.topic) == util.topic+"multiple":
        ...
        for i in range(len(topics)):
            ...
            if len(sample_timestamps[i])==1:
                ...
                data_to_append=[topics[i], '['+str(payloads[i])[2:-2]+']',str(corrected_ts)[3:-2] ,userid]
...
#database = db["temp"]# - document, we should use [test,light,etc.]
#database.drop()
#database = db["light"]# - document, we should use [test,light,etc.]
#database.drop()
